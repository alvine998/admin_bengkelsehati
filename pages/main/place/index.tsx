...
export async function getServerSideProps(context: any) {
    try {
        ...
        const result = await axios.get(CONFIG.base_url_api + `/places?pagination=true&search=${search || ""}&page=${page - 1 || 0}`, {
...
export default function Place({ table }: any) {
    ...
    const remove = async (e: any) => {
        ...
        setLoading(true)
        try {
            const result = await axios.delete(CONFIG.base_url_api + `/place?id=${formData?.id}`, {
                headers: { "bearer-token": 'bengkelsehati51' }
            })
            Swal.fire({
            ...
            </div>
            {
                modal.key == 'create' || modal.key == 'update' ?
                    <Modal
                        ...
                        title={modal.key == 'create' ? 'Tambah Data Lokasi' : 'Ubah Data Lokasi'}
                    >
                        <form onSubmit={save}>
                ...
                modal.key == 'delete' ?
                    ...
                    >
                        <form onSubmit={remove}>
                            <input type="text" className='hidden' value={modal.data.id} name='id' />
                            <p className='text-center'>Apakah anda yakin ingin menghapus lokasi {modal.data.name}?</p>
