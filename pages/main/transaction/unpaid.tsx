...
export async function getServerSideProps(context: any) {
    try {
        ...
        const result = await axios.get(CONFIG.base_url_api + `/transactions?pagination=true&search=${search || ""}&page=${page - 1 || 0}&status=unpaid`, {
        ...
        },
        {
            ...
            selector: (row: Transaction) => <>
                <button type='button' onClick={() => {
                    setModal({ ...modal, open: true, data: row, key: "approval" })
                }} >
                    <FaCheckCircle className='text-green-500 w-7 text-lg' />
                </button>
            </>
        }
    ]
                    ...
                    modal.key == 'approval' ?
                        ...
                        >
                            <form onSubmit={save}>
                                ...
                                <p className='text-center'>Apakah anda yakin ingin konfirmasi pembayaran {modal.data.code}?</p>
                                <div className='flex flex-col justify-center items-center my-4'>
                                    <label htmlFor="status">Status Pembayaran</label>
                                    <div id='status' className='flex gap-5'>
                                        <div className='flex gap-2'>
                                            <input type='radio' defaultChecked={modal?.data?.status === "paid"} value={'paid'} name='status' />
                                            <span>Terima</span>
                                        </div>
                                        <div className='flex gap-2'>
                                            <input type='radio' defaultChecked={modal?.data?.status === "unpaid"} value={'unpaid'} name='status' />
                                            <span>Tolak</span>
                                        </div>
                                    </div>
                                </div>
